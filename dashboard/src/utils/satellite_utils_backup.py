...
def get_image(
    ...
    max_attempts = int(get_kwargs.pop("max_attempts", 5)) if "get_kwargs" in locals() else 5
    attempt = 0
    while True:
        attempt += 1
        resp = requests.post(PROCESS_URL, json=payload, headers=headers, timeout=timeout)
        if resp.status_code == 429:
            ...
            wait_s = float(ra) if ra and str(ra).isdigit() else min(2 ** attempt, 10)
            logger.warning(f"⏳ 429 rate-limited, retry {attempt}/{max_attempts} in {wait_s}s")
